...
export const RegisterPage = () => {
  ...
  const PasswordRequirement = ({ met, text }: { met: boolean; text: string }) => (
    <div className={`flex items-center text-sm ${met ? 'text-green-600' : 'text-gray-400'}`}>
      <svg className={`w-4 h-4 mr-2 ${met ? 'text-green-500' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
        {met ? (
      ...
      <div className="max-w-2xl w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Create your account and start building secure smart contracts
          </p>
        </div>
              ...
              {formData.password && (
                <div className="mt-3 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-1">
                    ...
                    <PasswordRequirement met={passwordValidation.symbol} text="At least one symbol (!@#$%^&*)" />
                  </div>
                </div>
              )}
            </div>
